...
mark_task_completed() {
  ...
  sed -i='' "s|#   TASK ${sub_idx}: ${sub_name} \[ \]|#   TASK ${sub_idx}: ${sub_name} [✔]|" "$SCRIPT_ABS_PATH" && rm "$SCRIPT_ABS_PATH="
}
...
setup_find_file() {
  ...
  echo "Preparing a haystack of directories, please wait... (this may take a while)" | fold -s -w "$PRINT_WIDTH"
